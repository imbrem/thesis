...
The terminal model of `Isotope/LambdaCase/Models/Limits.lean` has singleton
...
boundary in `Isotope/LambdaCase/Models/Alg.lean`.
-/
...
theorem denote_constOps (S : Sig.{u}) (v : V) :
    ∀ {n : Nat} {β : BoundCtx S.Ty n} {t : Tm Empty S.Instr n} {A : S.Ty}
      (h : HasType S.Instr LambdaIter.Ctx.nil β t A), (constOps S v).denote h = v
...
theorem const_not_terminal (S : Sig.{u}) :
    ¬ Function.Injective
      (fun x : Bool => (default : const S true ⟶ terminal.{u, 0} S).map
        (n := 0) (β := LambdaIter.LocallyNameless.BoundCtx.nil)
        (A := LambdaIter.TypeFormers.unit) x) := by
  intro h
...
theorem constHom_ne_id (S : Sig.{u}) :
  ...
  have h' := congrArg
    (fun F : const S (true : Bool) ⟶ const S (true : Bool) =>
      F.map (n := 0) (β := LambdaIter.LocallyNameless.BoundCtx.nil)
        (A := LambdaIter.TypeFormers.unit) false) h
...
theorem powReindex_ne_id (S : Sig.{u}) :
  ...
  have h' := congrArg
    (fun F : pow Bool (const S (true : Bool)) ⟶ pow Bool (const S (true : Bool)) =>
      F.map (n := 0) (β := LambdaIter.LocallyNameless.BoundCtx.nil)
        (A := LambdaIter.TypeFormers.unit) (fun b => b) false) h
  exact Bool.noConfusion h'
