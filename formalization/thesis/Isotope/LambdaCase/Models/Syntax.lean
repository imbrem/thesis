...
/-!
# The syntactic model of lambda-case

`Syn S` is the quotient of the typable lambda-case terms over the signature `S`
by the equational theory `Equiv`, organized as an object of `Alg S` — an
algebra of the equational presentation.

Every operation is a `Quotient` lift of the corresponding term former; each is
well defined because `Equiv` has a congruence constructor for that former, with
no side conditions.  This is where the iteration-free setting pays: there is no
operation of lambda-case whose well-definedness on classes is in doubt.
...
Both are therefore true **by construction**, and are marked as such.  The
content of this file is `Syn.denote_mk`: the interpretation of a typing
derivation in `Syn S` is the class of that derivation.  Everything in
`Models/Initial.lean` rests on it.

## Honest boundary

This file gives `Syn S` as an *algebra of the presentation*.  It does **not**
build a premonoidal, distributive, or Freyd category out of the quotient, and
it makes no claim that the categorical interfaces of
`Isotope/LambdaCase/Semantics/Categorical.lean` give rise to such an algebra.
-/
...
def Carrier.inl (a : Carrier S β A) :
    Carrier S β (LambdaIter.TypeFormers.coprod A B) :=
  ⟨.inl a.1, a.2.elim fun ha => ⟨.inl ha⟩⟩

/-- Right injection on typable terms. -/
def Carrier.inr (b : Carrier S β B) :
    Carrier S β (LambdaIter.TypeFormers.coprod A B) :=
  ⟨.inr b.1, b.2.elim fun hb => ⟨.inr hb⟩⟩

/-- Case analysis on typable terms. -/
def Carrier.case (e : Carrier S β (LambdaIter.TypeFormers.coprod A B))
...
def Carrier.abort (a : Carrier S β LambdaIter.TypeFormers.empty) :
    Carrier S β C := ⟨.abort a.1, a.2.elim fun ha => ⟨.abort ha⟩⟩
...
def ops (S : Sig.{u}) : Alg.Ops.{u, u} S where
  ...
  pair := Quotient.map₂ Carrier.pair
    (fun _ _ ha _ _ hb => LocallyNameless.Equiv.pair ha hb)
  let₂ := Quotient.map₂ Carrier.let₂
    (fun _ _ ha _ _ hc => LocallyNameless.Equiv.let₂ ha hc)
  inl := Quotient.map Carrier.inl (fun _ _ h => LocallyNameless.Equiv.inl h)
  inr := Quotient.map Carrier.inr (fun _ _ h => LocallyNameless.Equiv.inr h)
  case := map₃ Carrier.case
    (fun _ _ he _ _ hl _ _ hr => LocallyNameless.Equiv.case he hl hr)
  abort := Quotient.map Carrier.abort
    (fun _ _ h => LocallyNameless.Equiv.abort h)
...
theorem ops_denote_eq_mk {n : Nat} {β : BoundCtx S.Ty n}
    ...
    (h : HasType S.Instr LambdaIter.Ctx.nil β t A) : (ops S).denote h = mk h := by
  induction h with
...
/-- **The syntactic model of lambda-case.**  Typable terms over the signature
`S`, modulo the equational theory `Equiv`, with every term former acting by its
congruence rule.

`coh` and `sound` are fields of `Alg`, and for `Syn` they hold *by
construction* — by proof irrelevance of the `Nonempty`-truncated typing
evidence, and by `Quotient.sound`, respectively — rather than as theorems about
lambda-case.  The theorem about lambda-case is `Syn.ops_denote_eq_mk`, and
through it `Syn.isInitial`. -/
...
@[simp] theorem let₂_mk {n : Nat} {β : BoundCtx S.Ty n} {A B C : S.Ty}
    ...
    (ha : HasType S.Instr LambdaIter.Ctx.nil β a
...
@[simp] theorem case_mk {n : Nat} {β : BoundCtx S.Ty n} {A B C : S.Ty}
    ...
    (he : HasType S.Instr LambdaIter.Ctx.nil β e
