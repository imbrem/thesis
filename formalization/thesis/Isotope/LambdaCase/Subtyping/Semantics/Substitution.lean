...
@[simp] theorem pull_succ {n : Nat} (β : BoundCtx τ n) (A : τ)
    ...
    pull (LambdaIter.Subtyping.LocallyNameless.TypedRenaming.succ β A) (ρ, a) = ρ := by
  induction β with
  | nil => rfl
  | snoc β B ih =>
      apply Prod.ext
...
@[simp] theorem pull_underBinder {n : Nat} (β : BoundCtx τ n) (X Y : τ)
    ...
    pull (LambdaIter.Subtyping.LocallyNameless.TypedRenaming.underBinder β X Y) ((ρ, x), y) = (ρ, y) := by
  apply Prod.ext
...
@[simp] theorem pull_underTwoBinders {n : Nat} (β : BoundCtx τ n) (X Y Z : τ)
    ...
    pull (LambdaIter.Subtyping.LocallyNameless.TypedRenaming.underTwoBinders β X Y Z)
      (((ρ, x), y), z) = ((ρ, y), z) := by
  apply Prod.ext
  · apply Prod.ext
    · exact pull_succ β X ρ x
...
theorem SubstDen.up {Γ : Ctx ν τ} {n k : Nat}
  ...
  · simp only [TypedSubst.up, Fin.cases_succ]
    change denote (m := m) (ε := ε)
      (HasType.rename (LambdaIter.Subtyping.LocallyNameless.TypedRenaming.succ β' A) (s j))
        γ (ρ', a) = _
    calc
      _ = denote (m := m) (ε := ε) (s j) γ
          (BoundDen.pull (LambdaIter.Subtyping.LocallyNameless.TypedRenaming.succ β' A) (ρ', a)) :=
...
@[simp] theorem denote_underTwoBinders {Γ : Ctx ν τ} {n : Nat} {β : BoundCtx τ n}
  ...
  let r := LambdaIter.Subtyping.LocallyNameless.TypedRenaming.underTwoBinders β X Y Z
