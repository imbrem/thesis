...
inductive Pure : Tm ν S → Prop where
  | var : Pure (.var x)
...
inductive Axiom : Tm ν S → Tm ν S → Prop where
...
inductive Eqv (S : Signature τ) : Ctx ν τ → Tm ν S → Tm ν S → τ → Prop where
  ...
  | let₂ (he : Eqv S Γ e e' (tensor A B))
      (hc : Eqv S (.snoc (.snoc Γ x A) y B) c c' C) :
  ...
  | case (he : Eqv S Γ e e' (coprod A B))
      (ha : Eqv S (.snoc Γ x A) a a' C)
      (hb : Eqv S (.snoc Γ y B) b b' C) :
