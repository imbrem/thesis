...
private def weakenSame (wΓ : FreeWk Γ' Γ) :
    ...
    {a b : Tm ν Φ n} → {A : τ} →
      Isotope.LambdaIter.LocallyNameless.Equiv pureEff Γ β a b A →
      Isotope.LambdaIter.LocallyNameless.Equiv pureEff Γ' β' a b A
...
def weaken (wΓ : FreeWk Γ' Γ) (wβ : BoundCtx.Wk β' β) (hAB : Subty A B) :
    Isotope.LambdaIter.LocallyNameless.Equiv (Φ := Φ) pureEff Γ β a b A →
    Isotope.LambdaIter.LocallyNameless.Equiv (Φ := Φ) pureEff Γ' β' a b B :=
...
theorem weaken_nonempty (wΓ : FreeWkProp Γ' Γ) (wβ : BoundCtx.WkProp β' β)
    ...
    (h : Nonempty (Isotope.LambdaIter.LocallyNameless.Equiv (Φ := Φ) pureEff Γ β a b A)) :
    Nonempty (Isotope.LambdaIter.LocallyNameless.Equiv (Φ := Φ) pureEff Γ' β' a b B) :=
