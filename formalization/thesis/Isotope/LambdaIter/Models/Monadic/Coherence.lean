...
/-!
# Coherence of the lambda-iter denotation in its typing derivation

Lambda-iter typing is *not* unique: `abort` types at every result type, and
...
theorem denote_coupled (M : Model.{u, v} S m) {n : Nat}
    ...
    ∀ {β₂ : BoundCtx S.Ty n} {A₂ : S.Ty}
      (k : HasType S.Instr Ctx.nil β₂ t A₂)
  ...
  | iter ha hb iha ihb =>
      ...
      | iter ka kb =>
          ...
          exact Coupled.iterate
            (u := fun x => denote M hb (ρ₁, x))
            (v := fun y => denote M kb (ρ₂, y))
            (fun q => ihb kb (ρ₁, q.val.1) (ρ₂, q.val.2) (hρ.snoc q.property))
            p
...
theorem denote_coh (M : Model.{u, v} S m) {n : Nat} {β : BoundCtx S.Ty n}
    ...
    (h k : HasType S.Instr Ctx.nil β t A) (ρ : M.Env β) :
