...
inductive Eqv (pureEff : ε) : Ctx ν τ → Tm ν Φ → Tm ν Φ → τ → Prop where
  ...
  | pair (ha : Eqv pureEff Γ a a' A) (hb : Eqv pureEff Γ b b' B) :
      Eqv pureEff Γ (.pair a b) (.pair a' b') (TypeFormers.tensor A B)
  | let₂ (he : Eqv pureEff Γ e e' (TypeFormers.tensor A B))
      (hc : Eqv pureEff (.snoc (.snoc Γ x A) y B) c c' C) :
      Eqv pureEff Γ (.let₂ x y e c) (.let₂ x y e' c') C
  | inl (h : Eqv pureEff Γ a a' A) :
      Eqv pureEff Γ (.inl a) (.inl a') (TypeFormers.coprod A B)
  | inr (h : Eqv pureEff Γ b b' B) :
      Eqv pureEff Γ (.inr b) (.inr b') (TypeFormers.coprod A B)
  | case (he : Eqv pureEff Γ e e' (TypeFormers.coprod A B))
  ...
  | iter (ha : Eqv pureEff Γ a a' A)
      (hb : Eqv pureEff (.snoc Γ x A) b b' (TypeFormers.coprod B A)) :
  ...
  | uniformity (hp : Pure pureEff h)
      ...
      (hsquare : Eqv pureEff (.snoc Γ (some x) A)
        ...
        (Tm.substSafe x' h b' hcapture') (TypeFormers.coprod B A')) :
