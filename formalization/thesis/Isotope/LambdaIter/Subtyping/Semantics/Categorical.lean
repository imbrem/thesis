...
class TypeModel (τ : Type u₃) [TypeFormers τ] [Subtyping τ]
  ...
  obj : τ → V
  tensorIso (A B : τ) : obj (TypeFormers.tensor A B) ≅ obj A ⊗ obj B
  unitIso : obj (TypeFormers.unit : τ) ≅ 𝟙_ V
  coprodIso (A B : τ) : obj (TypeFormers.coprod A B) ≅ obj A ⨿ obj B
  emptyIsInitial : IsInitial (obj (TypeFormers.empty : τ))
...
noncomputable def abort {τ : Type u₃} [TypeFormers τ] [Subtyping τ]
    ...
    (emptyComputation : J.obj R ⟶ J.obj (M.obj (TypeFormers.empty : τ))) :
