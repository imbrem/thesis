...
`pair`, `caseWithContext` or `contextualLoop` directly; they mention terms.  But
every reduction of a scheme passes through equations between those combinators,
and this file establishes them once, in an arbitrary Freyd category, before any
syntax is involved.
