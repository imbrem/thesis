...
/-- Lexical equivalence generated by coherent reindexing of CFG binders and
closed under every region constructor. -/
inductive ReindexEqv : Region Φ → Region Φ → Prop where
  | refl (r : Region Φ) : ReindexEqv r r
  | symm : ReindexEqv r s → ReindexEqv s r
  | trans : ReindexEqv r s → ReindexEqv s t → ReindexEqv r t
  | case : ReindexEqv l l' → ReindexEqv r r' →
      ReindexEqv (.case discr l r) (.case discr l' r')
  | let₁ : ReindexEqv r r' → ReindexEqv (.let₁ value r) (.let₁ value r')
  | let₂ : ReindexEqv r r' → ReindexEqv (.let₂ value r) (.let₂ value r')
  | cfgChildren (entry : Region Φ) (arity : Nat)
      (left right : Fin arity → Region Φ)
      (children : ∀ i, ReindexEqv (left i) (right i)) :
      ReindexEqv (.cfg entry arity left) (.cfg entry arity right)
  | cfgReindex {n : Nat} {R R' : Fin n → τ} {L : LCtx τ}
      (p : CfgReindexing R R' L) (entry : Region Φ) (blocks : Fin n → Region Φ) :
      ReindexEqv (.cfg entry n blocks) (p.apply entry blocks)
