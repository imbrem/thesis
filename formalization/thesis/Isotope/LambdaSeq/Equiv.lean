...
def Eqv [DecidableEq ν] [LambdaIter.TypeFormers τ] [LambdaIter.Subtyping τ]
  ...
  LambdaCase.Named.Eqv pureEff Γ (Tm.embedCase a) (Tm.embedCase b) A

theorem Eqv.embedCase [DecidableEq ν] [LambdaIter.TypeFormers τ] [LambdaIter.Subtyping τ]
    [LambdaIter.HasTy Φ τ] [LambdaIter.HasEff Φ ε]
    {pureEff : ε} {Γ : Ctx ν τ} {a b : Tm ν Φ} {A : τ} :
    Eqv pureEff Γ a b A →
      LambdaCase.Named.Eqv pureEff Γ (Tm.embedCase a) (Tm.embedCase b) A := id
...
/-- The typed monadic theory generated by beta/eta and associativity for `let`. -/
...
def Equiv.embedIter {pureEff : ε} {Γ : Ctx ν τ} {n : Nat} {β : BoundCtx τ n}
    {a b : Tm ν Φ n} {A : τ} : Equiv pureEff Γ β a b A →
    LambdaIter.LocallyNameless.Equiv pureEff Γ β a.embedIter b.embedIter A :=
