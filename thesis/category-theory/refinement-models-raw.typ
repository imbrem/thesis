...
== Models of $lambda_(sans(i t e r))$
...
to the features of $lambda_(sans(i t e r))$. In particular, we need to
...
general,
$\( f ⊗ sans(i d) \) ; \( sans(i d) ⊗ g \) ≠ \( sans(i d) ⊗ g \) ; \( f ⊗ sans(i d) \)$.
...
$sans(p r i n t) : sans(s t r) arrow.r \( \)$,
$\( sans(p r i n t) ⊗ sans(i d) \) ; \( sans(i d) ⊗ sans(p r i n t) \) ≠ \( sans(i d) ⊗ sans(p r i n t) \) ; \( sans(p r i n t) ⊗ sans(i d) \)$
since, given input $(mono("hello"), mono("world"))$, the
left-hand side will print $mono(h e l l o w o r l d)$, while the
right-hand side will print $mono(w o r l d h e l l o)$. Note that the
...
$y : bb(Z) + sans(s t r)$, consider the following expression:
$ sans(c a s e) #h(0em) y #h(0em) { iota_l #h(0em) y : sans(p r i n t) \( mono("\"add: \"") \, x + y \) \, iota_r #h(0em) y : sans(p r i n t) \( y \, x \) } $
While our input context corresponds to the object
$bb(Z) ⊗ \( bb(Z) + sans(s t r) \)$, we need to somehow get
to
$\( bb(Z) ⊗ bb(Z) \) + \( bb(Z) ⊗ sans(s t r) \)$
...
- #emph[Naturality:] given $f : A arrow.r B + A$ and $g : B arrow.r C$,
  we have $\( f ; g + sans(i d) \)^dagger = f^dagger ; g : A arrow.r C$

- #emph[Dinaturality:] given morphisms $g : A arrow.r B + C$ and
  $h : C arrow.r B + A$, we have that
  $\( g ; \[ iota_l \, h \] \)^dagger = g ; \[ sans(i d)_B \, \( h ; \[ iota_l \, g \] \)^dagger \]$

- #emph[Codiagonal:] given $f : A arrow.r \( B + A \) + A$, we have
  $\( f^dagger \)^dagger = \( f ; \[ sans(i d) \, iota_r \] \)^dagger : A arrow.r B$

]
#todo[Mechanically port the four TikZ string diagrams from source lines 1957--2100. Pandoc preserved their captions and labels below but omitted the TikZ drawing bodies.]
...
$ \( sans(i t e r) #h(0em) a #h(0em) { iota_r #h(0em) x : b } \, c \) approx sans(i t e r) #h(0em) \( a \, c \) #h(0em) { iota_r #h(0em) \( x \, y \) : sans(c a s e) #h(0em) b #h(0em) { iota_l #h(0em) z : iota_l #h(0em) \( z \, y \) \, iota_r #h(0em) z : iota_r #h(0em) \( z \, y \) } } $
...
effectful $lambda_(sans(i t e r))$ programs, but we still need to be
...
justify that
$sans(l e t) #h(0em) x = a ; #h(0em) \( b \, x \) approx \( b \, a \)$
proving that
$sans(l e t) #h(0em) x = a ; #h(0em) sans(i t e r) #h(0em) b #h(0em) { iota_r #h(0em) y : c } approx sans(i t e r) #h(0em) b #h(0em) { iota_r #h(0em) y : sans(l e t) #h(0em) x = a ; #h(0em) c }$
for $x in.not sans(f v) \( b \)$ requires us to be able to move a
...
$c = sans(i d)_C$) whenever $s$ is in $cal(K)$ and $b$ is in $cal(C)$.
...
idea, we can now define a $lambda_(sans(i t e r))$-model as follows:
$lambda_(sans(i t e r))$:

#block[
A model
$cal(M) = \( cal(C) \, \( dot.op \)^dagger \, ⟦ dot.op ⟧ \, Delta_() \, !_() \)$
of a $lambda_(sans(i t e r))$-signature
...
- For each function
  $f : sans(I n s t) \( cal(S) \)_epsilon.alt \( A \, B \)$, a morphism
...
- $⟦ upright(bold(1)) ⟧ = I$,
  $⟦ A ⊗ B ⟧ = ⟦ A ⟧ ⊗ ⟦ B ⟧$,
  $!_(upright(bold(1))) = sans(i d)_I$, and
  $Delta_(upright(bold(1))) = lambda^(- 1) = rho^(- 1)$

- For $A \, B$ affine,
  $!_(A ⊗ B) = !_A ⊗ !_B ; lambda$, and for
  $A \, B$ relevant,
  $Delta_(A ⊗ B) = Delta_A ⊗ Delta_B ; sigma^(sans(m i d))$
  where we define
  $sigma_(A \, B \, C \, D)^(sans(m i d)) = alpha_(A ⊗ \( B ⊗ C \) ⊗ D) ; A ⊗ sigma ⊗ D ; alpha$
  having type
...
#figure([#block[
  ...
  ⟦ dot.op mapsto dot.op ⟧ = sans(i d)_I #h(2em) ⟦ Gamma^(upright(bold(q))) \, x : A_epsilon.alt^q mapsto Delta^(upright(bold(q))') ⟧ = ⟦ Gamma^(upright(bold(q))) ⟧ ⊗ !_(A^q) ; lambda ; ⟦ Gamma^(upright(bold(q))) mapsto Delta^(upright(bold(q))') ⟧\
  ⟦ Gamma^(upright(bold(q))) \, x : A_epsilon.alt^q mapsto Delta^(upright(bold(q))') \, x : A_(epsilon.alt')^(q') ⟧ = ⟦ Gamma^(upright(bold(q))) mapsto Delta^(upright(bold(q))') ⟧ ⊗ cases(delim: "{", sans(i d)_(⟦ A ⟧) & upright("if ") q \, q' ≠ 0, !_(A^q) & upright("otherwise"), ) $
  ...
  ⟦ Gamma \, x : A tack.r \( upright(bold(q)) \, q \) = \( upright(bold(q))_l \, q_l \) + \( upright(bold(q))_r \, q_r \) ⟧ = ⟦ Gamma tack.r upright(bold(q)) = upright(bold(q))_l + upright(bold(q))_r ⟧ ⊗ (cases(delim: "{", lambda^(- 1) & upright("if ") q_l = 0 upright(" else"), rho^(- 1) & upright("if ") q_r = 0 upright(" else"), Delta_A & upright("otherwise"))) ; sigma^(sans(m i d)) $

  ]],
  caption: [
    Denotational semantics for structural $lambda_(sans(i t e r))$
    judgements
  ]
)
<fig:struct-sem>
