...
sans("PW")_(sans(L)) ( sans("let") #h(0em) ( x   y ) = a ; r ) & = sans("let") #h(0em) ( x   y ) = a ; sans("PW")_(sans(L)) ( r )\
sans("PW")_(sans(L)) ( sans("case") #h(0em) e #h(0em) { iota_l #h(0em) x : r   iota_r #h(0em) y : s } ) & = sans("case") #h(0em) e #h(0em) { iota_l #h(0em) x : sans("PW")_(sans(L)) ( r )   iota_r #h(0em) y : sans("PW")_(sans(L)) ( s ) }\
sans("PW")_(sans(L)) ( r #h(0em) sans("where") #h(0em) ( ell_i ( x_i ) : { t_i }   )_i ) & = sans("seq") ( sans("PW")_(sans(L)) ( r )   sans("case") #h(0em) sans("ua") #h(0em) square.stroked.tiny #h(0em) { iota_l #h(0em) x : sans("br") #h(0em) square.filled.medium #h(0em) x\
...
- The $r #h(0em) sans("where") #h(0em) ( ell_i ( x_i ) : { t_i }   )_i$ case, in which we replace the set of labels bound by the $sans("where")$ clause with a tag and a while loop containing a case statement branching on the tag. (This uses the $sans("ua")$ function, which implements associativity of $n$-ary coproducts. That is, given $Gamma tack.r_tack.t e : \[ L   R \]$, we have that $Gamma tack.r_tack.t sans("ua")_(sans(L)) #h(0em) e : \[ L \] + \[ R \]$.)
...
- $alpha_(A   B   C)^R$ is natural: we have that, given $f : R ⊗ A arrow.r A'$, $g : R ⊗ B arrow.r B'$, $h : R ⊗ C arrow.r C'$ $ ( f ⊗_R B ) ⊗_R C gt.double_() alpha_(A'   B   C) & = alpha_(A   B   C) gt.double_() f ⊗_R ( B ⊗ C )\
  ...
  ( A ⊗ B ) ⊗_R h gt.double_() alpha_(A   B   C') & = alpha_(A   B   C) gt.double_() A ⊗_R ( B ⊗_R h )\
   $<eqn:env-a-nat-1> since, for each equation, both sides are equal to the same string diagram in Figure~#todo[Cross-reference: \@fig:env-a-nat-1], #todo[Cross-reference: \@fig:env-a-nat-2], and #todo[Cross-reference: \@fig:env-a-nat-3] respectively.
...
- Pure morphisms are central: given $f : R ⊗ A arrow.r A'$ and $g : R ⊗ B arrow.r B'$ pure, we have that $ f ⊗_R B gt.double_() A' ⊗_R g & = Delta_R ; alpha ; R ⊗ ( alpha^(- 1) ; f ⊗ B ; sigma ) ; alpha^(- 1) ; g ⊗ A' ; sigma\
   ...
   & = A ⊗_R g gt.double_() f ⊗_R B' $ since both sides correspond to the diagram in Figure~#todo[Cross-reference: \@fig:env-slide.] We will write this as $f ⊗_R g$.
