...
- First, we give a type-theoretic presentation of SSA, with both typing rules (in Section~#conditional-ref("sec:typing")) and an equational theory (in Section~#conditional-ref("sec:equations")) for well-typed terms. We also prove the correctness of suitable substitution properties for this calculus.
...
- We also show, in Section~#conditional-ref("ssec:completeness"), that syntax quotiented by the equational theory yields the initial distributive Elgot category. This establishes that our set of syntactic equations is complete, and that there are no equations which the denotational semantics validates, but which cannot be proved syntactically.
